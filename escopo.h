...
{                              /* Dados relativos a tabela turma */
    ...
    char codigoDisciplina[15]; // Codigo da turma: exe.: TCCOMP12
} tTurma;
...
// Função para salvar os dados inseridos no arquivo Database.dat
void salvarArq()
{
    FILE *arq = fopen("Database.dat", "a+"); // Função fopen abre o arquivo para leitura e escrita
    int i, j, k;

    if (arq) // Se o arquivo foi aberto com sucesso faz a gravação dos novos dados
    {
    ...
    fclose(arq); // Função fclose fecha o arquivo
}
